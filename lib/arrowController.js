...
class Arrow {
  constructor(options = {}) {
    ...
    this._options.scale = this._options.scale || window.devicePixelRatio || 1;
    this._options.fillColor = this._options.fillColor || '#000';

    this._line = new fabric.Line([0,0,0,0], {
      stroke: this._options.fillColor,
      selectable: true,
      strokeWidth: 5 * this._options.scale,
      hasBorders: false,
      hasControls: false,
      originX: 'center',
      originY: 'center',
    ...
    const deltaX = this._line.left - centerX;
    const deltaY = this._line.top - centerY;
    ...
    this._arrow = new fabric.Path(arrowPath, {
      ...
      angle: -45,
      width: 20 * this._options.scale,
      height: 20 * this._options.scale,
      fill: this._options.fillColor,
      className: this,
    });
    this._arrow.parentObject = this;
    this._arrow.line = this._line;

    this._line.arrow = this._arrow;
    this._line.customType = this._arrow.customType = 'arrow';
    ...
    this._calcArrowAngle = this._calcArrowAngle.bind(this);
    this._moveEnd = this._moveEnd.bind(this);
    this._moveLine = this._moveLine.bind(this);
  ...
  _moveEnd(obj) {
    ...
    const x1 = obj.line.get('x1');
    const y1 = obj.line.get('y1');
    const x2 = obj.line.get('x2');
    const y2 = obj.line.get('y2');
    const angle = this._calcArrowAngle(x1, y1, x2, y2);
  ...
  _moveLine(obj) {
    const oldCenterX = (obj.x1 + obj.x2) / 2;
    const oldCenterY = (obj.y1 + obj.y2) / 2;
  ...
  _onMouseDown(that, o) {
    ...
    const oldCenterX = (this._line.x1 + this._line.x2) / 2;
    const oldCenterY = (this._line.y1 + this._line.y2) / 2;
    const deltaX = this._line.left - oldCenterX;
    const deltaY = this._line.top - oldCenterY;
    const angle = this._calcArrowAngle(this._line.x2, this._line.y2, this._line.x1, this._line.y1);
    ...
    that.canvas.add(this._line, this._arrow);
  ...
  _onMouseMove(that, o) {
    ...
    const pointer = that.canvas.getPointer(o.e);
  ...
  _onMouseUp(that, o) {
    ...
    this._line.arrow.on('moving', () => {
      this._moveEnd(this._line.arrow);
      that.canvas.renderAll();
    });

    this._line.on('moving', () => {
      this._moveLine(this._line);
    });
  }
}
