...
class Arrow {
  constructor(options = {}) {
    ...
    this._line.delete = this._arrow.delete = () => this.delete();
  ...
  _moveEnd() {
    ...
    this._line._setWidthHeight();
    this._line.setCoords();

    this._arrow.set('angle', this._calcArrowAngle(this._line.x2, this._line.y2, this._line.x1, this._line.y1));
  }
  ...
  _moveLine() {
    ...
    this._line.set({
      'x1': this._line.x1 + deltaX,
      'y1': this._line.y1 + deltaY,
      'x2': this._line.x2 + deltaX,
      'y2': this._line.y2 + deltaY,
    });

    this._line.set({
      'left': (this._line.x1 + this._line.x2) / 2,
      'top': (this._line.y1 + this._line.y2) / 2,
    });
  }

  _onMouseDown(that, o) {
    this._isMouseDown = true;
    this._mousePosition = that.canvas.getPointer(o.e);
    this._line.set({
      'x1': this._mousePosition.x,
      'y1': this._mousePosition.y,
      'x2': this._mousePosition.x,
      'y2': this._mousePosition.y,
    }).setCoords();
