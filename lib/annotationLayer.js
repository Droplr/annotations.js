...
class AnnotationLayer {
  ...
  initCanvasHistoryState() {
    ...
    this.history.state[0] = canvasAsJson;
  }

  updateCanvasState() {
    if ((!this.history.undoAvailable && !this.history.redoAvailable)) {
      const jsonData = this.canvas.toJSON();
      const canvasAsJson = JSON.stringify(jsonData);
  ...
  undoAction() {
    ...
    if (!this.history.undoFinished) return;

    if (this.history.currentStateIndex === 0) {
      this.history.undoAvailable = false;
    } else if (this.history.state.length >= 1) {
      this.history.undoFinished = false;

      if (this.history.currentStateIndex >= 0) {
        this.history.undoAvailable = true;
        this.history.currentStateIndex = this.history.currentStateIndex > 0 ? this.history.currentStateIndex - 1 : 0;
        this.canvas.loadFromJSON(this.history.state[this.history.currentStateIndex],
          () => {
            this.canvas.renderAll();
            this.history.undoAvailable = false;
  ...
  redoAction() {
    if (!this.history.redoFinished) return;

    if (this.history.state.length && (this.history.state.length > this.history.currentStateIndex)) {
      this.history.redoFinished = false;
      this.history.redoAvailable = true;

      this.canvas.loadFromJSON(this.history.state[this.history.currentStateIndex + 1], () => {
        this.canvas.renderAll();
  ...
  */
  calculateSize() {
    ...
    */
    const width = this.imageWidth > 2500 ? this.imageWidth : 2500;
    const height = this.imageHeight > 2500 ? this.imageHeight : 2500;
  ...
  findEdgePoints() {
    ...
    */
    for (let i = 0; i < size; i++) {
      ...
      if (fragment == 'arrow') return edgePoints;
